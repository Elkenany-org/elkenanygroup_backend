...
<div class="card-styles">
    ...
    <div class="card-style-3 mb-30">
        ...
        @php
            $header3 = Route::currentRouteName() === 'homeheader3.show' && 
                request()->query('page_name') === $header->page_name && 
                request()->query('type') === 'header3';
            $header2 = Route::currentRouteName() === 'homeheader2.show' && 
                request()->query('page_name') === $header->page_name && 
                request()->query('type') === 'header2';
            $header1 = Route::currentRouteName() === 'homeheader1.show' && 
                request()->query('page_name') === $header->page_name && 
                request()->query('type') === 'header1';
        @endphp
        <a class="main-btn primary-btn btn-hover w-25 text-center m-3 {{ $header3 ? 'active' : '' }}" href="{{ route('homeheader3.show',['page_name' => 'home', 'type' => 'header3']) }}">Header 3</a>
        <a class="main-btn primary-btn btn-hover w-25 text-center m-3 {{ $header2 ? 'active' : '' }}" href="{{ route('homeheader2.show',['page_name' => 'home', 'type' => 'header2']) }}">Header 2</a>
        <a class="main-btn primary-btn btn-hover w-25 text-center m-3 {{ $header1 ? 'active' : '' }}" href="{{ route('homeheader1.show',['page_name' => 'home', 'type' => 'header1']) }}">Header 1</a>
        @endif
        </div>
